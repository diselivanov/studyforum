import { zUpdateTopicTrpcInput } from '@studyforum/backend/src/router/topics/updateTopic/input'
import { canEditTopic } from '@studyforum/backend/src/utils/can'
...
export const EditTopicPage = withPageWrapper({
  authorizedOnly: true,
  useQuery: () => {
    const { selectedTopic } = getEditTopicRoute.useParams()
    return trpc.getTopic.useQuery({
      selectedTopic,
    })
  },
  setProps: ({ queryResult, ctx, checkExists, checkAccess }) => {
    const topic = checkExists(queryResult.data.topic, 'Тема не найдено')
    checkAccess(canEditTopic(ctx.me, topic), 'Отказано в доступе')
    return {
      topic,
    }
  },
  ...
  const { formik, buttonProps, alertProps } = useForm({
    ...
    validationSchema: zUpdateTopicTrpcInput.omit({ topicId: true }),
    onSubmit: async (values) => {
      await updateTopic.mutateAsync({ topicId: topic.id, ...values })
      navigate(getViewTopicRoute({ selectedTopic: topic.id }))
    },
